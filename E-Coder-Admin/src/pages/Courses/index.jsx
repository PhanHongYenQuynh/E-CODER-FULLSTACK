...
const ListCourse = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await fetch(`${LINK_API}/course/list-all`);
  ...
  const handleDelete = async (courseId) => {
    try {
      ...
      const response = await fetch(`${LINK_API}/course/${courseId}`, {
        method: 'DELETE',
        headers: {
  ...
  return (
    ...
    dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
        ...
        <button type="button" onClick={()=> navigate('/admin/add-course',{replace: true})} className="ml-3 text-gray-900 bg-white border border-gray-300 focus:outline-none hover:bg-gray-100 focus:ring-4 focus:ring-gray-200 font-medium rounded-lg text-sm px-5 py-2.5 me-2 mb-2 dark:bg-gray-800 dark:text-white dark:border-gray-600 dark:hover:bg-gray-700 dark:hover:border-gray-600 dark:focus:ring-gray-700">Create</button>
        <div className="px-3 py-4 flex justify-center">
          <table className="w-full text-md bg-white shadow-md rounded mb-4 overflow-x-scroll">
            <tbody>
              <tr className="border-b">
                <th className="text-left p-2 px-3">Id Course</th>
                <th className="text-left p-2 px-3">Name</th>
                <th className="text-left p-2 px-3">Image</th>
                <th className="text-left p-2 px-3">Price</th>
                <th className="text-left p-2 px-3">Time</th>
                <th className="text-left p-2 px-3">Description</th>
                <th className="text-left p-2 px-3">Chapter</th>
              ...
              {filterCourse?.map((item, index) => (
                <tr key={item._id} className="border-b hover:bg-orange-100">
                  ...
                  <td className="p-2 px-3">
                    <img src={item.banner} className='h-[100px] w-[100px] object-cover' alt="" />
                  </td>
                  <td className="p-2 px-3">
                  {item.price}
                  </td>
                  ...
                  <td className="p-2 px-3">
                  {item.chapterId?.map((item) =>(
                    <span className="bg-green-100 text-green-800 text-xs font-medium me-2 px-3.5 py-1.5 rounded-full dark:bg-green-900 dark:text-green-300">{item.title}</span>
                  ))}
                  </td>
                  ...
                  <td className="p-2 px-3 flex justify-end">
                    <button
                      type="button"
                      onClick={() => navigate(`/admin/edit-course/${item._id}`)}
